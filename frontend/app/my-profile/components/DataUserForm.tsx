  ...
  const handleClick = (e) => {
    ...
    <form onSubmit={handleClick} className="w-full mx-24 my-auto border flex flex-col h-[450px] p-6 rounded-lg shadow-sm duration-500">
      <label className="mb-4">
        ...
        <input disabled type="text" value={`${user?.nom}`} className="px-2 rounded border-2 border-solid border-black form-input mt-1 block w-full focus:text-white focus:bg-black" />
      </label>


      <label className="mb-4">
        <strong>Email:</strong>
        <input disabled type="text" value={`${user?.email}`} className="px-2 rounded border-2 border-solid border-black form-input mt-1 block w-full focus:text-white focus:bg-black" />
      </label>

      <label className="mb-4">
        <strong>Mot de passe:</strong>
        <input disabled type="password" value={`${user?.mdp}`} className="px-2 rounded border-2 border-solid border-black form-input mt-1 block w-full focus:text-white focus:bg-black" />
      </label>
        ...
        <span id="profil-picture" className="text-white rounded cursor-pointer inline-block p-2 mx-auto flex justify-center group opacity-0 duration-500">
            ...
            className="w-28 h-28 mx-auto group-hover:opacity-20"
                        />
            <svg  className="hidden group-hover:block w-8 h-8 absolute mx-auto bottom-[375px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
